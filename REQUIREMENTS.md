...
| T-02 | 创建课程时设置：课程名称、最大行数（≤20）、最大列数（≤20）、学生名单 | P0 |
...
| T-18 | 教师可以**手动代签**，从**未签到学生列表中选择**，只需输入行和列 | P2 ✅ |
...
| S-04 | 签到表单字段：**学号**、**姓名**、**行**（下拉 1~max）、**列**（下拉 1~max） | P0 ✅ |
| S-05 | 前端校验：所有字段必填，行/列在范围内 | P0 ✅ |
...
|------|------|------|
| 口令验证 | `/student` | 输入课程口令 |
| 签到表单 | `/student/checkin/:sessionId` | 填写学号、姓名、行、列 |
...
├── max_row     INTEGER NOT NULL DEFAULT 10 — 最大行数 (≤20)
